...
// Gate for an incoming preview request: 0 if its port may be served, -1 with
// a message in `err` otherwise. The allowlist is process-local, so this must
// run in the daemon — bridge_preview_handle_request executes in a worker
// process that never saw the registration and assumes it was already checked.
